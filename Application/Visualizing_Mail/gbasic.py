'''
    做两个简单的数据分析：“谁发送邮件最多？”和“哪个组织发送邮件最多？”

    与gmane.py和gmodel.py相比，gbasic.py处理数据非常快。虽然它们都在相同的数据上工作，但gbasic.py使用index.sqlite中压缩过和规范化的数据。
如果有大量数据需要管理，本节示例应用程序采用的多步处理可能多用了一些开发时间，但在数据探索与可视化时节省了大量时间。'''
