'''
    数据获取：   https://online.dr-chuck.com/files/sakai/email/content.sqlite.zip
1.在gmane.py中基础URL是硬编码的，被硬编码为Sakai开发者列表。通过修改基础URL可以爬取其他归档库。如果修改了基础URL，请确保删除content.sqlite文件。
2.程序扫描content.sqlite，从1开始，直到找到未被爬取的消息的序号，然后开始爬取那条消息。直到爬取到需要的消息序号或者访问到一个不符合消息格式的页面，程序终止。
3.一旦爬取了所有消息并将它们存储在content.sqlite，你可以再次执行gmane.py来获取邮件列表上新发布的消息
'''
