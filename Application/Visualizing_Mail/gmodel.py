'''
1.程序从content.sqlite读入原始数据，进行数据清理与建模，生成index.sqlite文件。由于压缩了标题和正文，index.sqlite比content.sqlite文件体积一般要小十倍
2.每次执行gmodel.py，它都会删除和重建index.sqlite，允许调整参数和编辑content.sqlite里的映射表，从而控制数据清洗过程
3.程序主要是执行一些数据清理任务。
'''
...
def fixsender(sender,allsenders=None) :
    ...
    # Check if we have a hacked Visualizing_Mail.org from address
    if allsenders is not None and sender.endswith('Visualizing_Mail.org') :
        pieces = sender.split('-')
        realsender = None
        for s in allsenders:
...
for message_row in cur_1 :
    ...
    if 'Visualizing_Mail.org' in sender : continue
...
for message_row in cur_1 :
    ...
    if 'Visualizing_Mail.org' in sender:
